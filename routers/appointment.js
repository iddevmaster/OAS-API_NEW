...
let _check_user = await runQuery(
  ...
  while (currentDate <= endDate) {
    ...
    if(LoaDay.select == true){
      ...
      let sqlx = "SELECT A.group_id,A.group,A.users_id,B.name,B.province_code,C.province_name FROM app_group_users A LEFT JOIN app_group B ON A.group = B.group_id LEFT JOIN app_zipcode_lao C ON C.province_code = B.province_code WHERE A.users_id = ? GROUP BY A.group_id,A.users_id";
      ...
      if(getContent[0]?.numRows == 0){
        ...
        [2, data.ap_quota,data.ap_date_start,data.ap_date_end,currentDate.toISOString().split('T')[0],'-','-',functions.dateAsiaThai(),functions.dateAsiaThai(), user_id,user_id,time,group_id,LoaDay.days,results[0].province_name]
...
router.post("/listall", middleware, async (req, res, next) => {
  ...
  if(_check_user[0].user_type == '1'){
    ...
    let sql = `
    SELECT A.day,A.province_code,A.ap_id,SUM(A.ap_quota) As quata,A.time,A.ap_date_first,F.user_firstname,F.user_lastname,A.peop_addrs,GROUP_CONCAT(B.dlt_code ORDER BY B.dlt_code SEPARATOR '/') AS dlt,IFNULL(E.order_count, 0) AS available from app_appointment A LEFT JOIN app_appointment_type B ON A.ap_id = B.ap_id 
...
router.get("/event/newlist", middleware, async (req, res, next) => {
  ...
  con.query(
    "SELECT A.ap_id,A.ap_quota,A.ap_date_first,B.dlt_code,A.time,IFNULL(E.order_count, 0) AS available,F.province_code,F.province_name,A.group_id,G.name from app_appointment A LEFT JOIN app_appointment_type B ON A.ap_id = B.ap_id LEFT JOIN (select ap_id,dlt_code,COUNT(*) AS order_count  from app_appointment_reserve o GROUP BY o.ap_id,o.dlt_code) E ON A.ap_id = E.ap_id AND B.dlt_code = E.dlt_code LEFT JOIN app_group G ON G.group_id = A.group_id LEFT JOIN app_zipcode_lao F ON G.province_code = F.province_code where B.dlt_code = ? AND A.ap_date_first >= ? and A.ap_date_first <= ? AND A.group_id = ? GROUP BY A.ap_id",
    [dlt_code,present_day,last_day,group_id],
    (err, result) => {
      if (err) {
        return res.status(400).json({
          status: 400,
...
router.get("/reserve/get/:user_id", middleware, (req, res, next) => {
  ...
  let sql = `
  SELECT t1.*, t2.ap_date_first,t2.type,t2.group_id,t6.name,t6.province_code,t7.province_name,t2.time,
...
router.post("/dateappointmentlist", middleware, async (req, res, next) => {
  ...
  if(user_type == 1){
    let sql = `select A.ar_id,A.dlt_code,B.ap_date_first,B.time,B.group_id,F.name,E.province_name,A.user_id,A.ap_number,A.id_card,A.st_id,A.app_status,(SELECT mr_score FROM app_main_result WHERE user_id= A.user_id AND mr_learn_type = 1 AND dlt_code = A.dlt_code) AS thero,(SELECT mr_score FROM app_main_result WHERE user_id= A.user_id AND mr_learn_type = 2 AND dlt_code = A.dlt_code) AS pratic,(select mr_status from app_main_result  WHERE user_id= A.user_id AND mr_learn_type = 1 AND dlt_code = A.dlt_code) AS mr_status_t,(select mr_status from app_main_result  WHERE user_id= A.user_id AND mr_learn_type = 2 AND dlt_code = A.dlt_code) AS mr_status_p,C.*,D.*
from app_appointment_reserve A 
LEFT JOIN app_appointment B ON A.ap_id = B.ap_id
LEFT JOIN app_user C ON C.user_id = A.user_id
LEFT JOIN app_user_detail D ON C.user_id = D.user_id
LEFT JOIN app_zipcode_lao E ON E.id = D.location_id
LEFT JOIN app_group F ON B.group_id = F.group_id
