...
router.get("/list?", middleware, (req, res, next) => {
  ...
  con.query(
    "SELECT  A.mr_id,A.dlt_code,A.mr_learn_type,A.mr_score,A.mr_status,A.crt_date AS time,B.*,C.ap_number,D.user_full  FROM app_main_result A LEFT JOIN app_user B ON A.user_create = B.user_id LEFT JOIN app_appointment_reserve C ON C.ap_number = A.ap_number LEFT JOIN app_appointment D ON D.ap_id = C.ap_id WHERE A.user_id  = ? ORDER BY mr_id DESC",
    [user_id],
    (err, rows) => {
